-- MOJE TŘÍDA — PHASE 2
-- Versioned curriculum source-of-truth foundation.
-- No AI-generated lesson content is stored here.
...
create trigger curriculum_outcomes_set_updated_at before update on public.curriculum_outcomes
for each row execute function public.set_updated_at();
create trigger school_curriculum_versions_set_updated_at before update on public.school_curriculum_versions
for each row execute function public.set_updated_at();
...
create policy curriculum_sources_select_authenticated on public.curriculum_sources
for select to authenticated using (true);
create policy curriculum_versions_select_published on public.curriculum_versions
for select to authenticated using (status = 'published');
create policy curriculum_areas_select_published on public.curriculum_areas
for select to authenticated using (exists (
  select 1 from public.curriculum_versions v where v.id = curriculum_version_id and v.status = 'published'
));
create policy curriculum_subjects_select_published on public.curriculum_subjects
for select to authenticated using (exists (
  select 1 from public.curriculum_versions v where v.id = curriculum_version_id and v.status = 'published'
));
create policy curriculum_topics_select_published on public.curriculum_topics
for select to authenticated using (exists (
  select 1 from public.curriculum_versions v where v.id = curriculum_version_id and v.status = 'published'
));
create policy curriculum_outcomes_select_published on public.curriculum_outcomes
for select to authenticated using (exists (
  select 1 from public.curriculum_versions v where v.id = curriculum_version_id and v.status = 'published'
));
create policy curriculum_dependencies_select_published on public.curriculum_dependencies
for select to authenticated using (exists (
  select 1 from public.curriculum_versions v where v.id = curriculum_version_id and v.status = 'published'
));
...
create policy school_curriculum_versions_select_member on public.school_curriculum_versions
for select to authenticated using (public.is_school_member(school_id));
create policy school_curriculum_versions_insert_admin on public.school_curriculum_versions
for insert to authenticated with check (public.is_school_admin(school_id));
create policy school_curriculum_versions_update_admin on public.school_curriculum_versions
for update to authenticated using (public.is_school_admin(school_id)) with check (public.is_school_admin(school_id));
create policy school_curriculum_versions_delete_admin on public.school_curriculum_versions
for delete to authenticated using (public.is_school_admin(school_id));

create policy school_curriculum_mappings_select_member on public.school_curriculum_mappings
for select to authenticated using (exists (
  select 1 from public.school_curriculum_versions scv
  where scv.id = school_curriculum_version_id and public.is_school_member(scv.school_id)
));
create policy school_curriculum_mappings_write_admin on public.school_curriculum_mappings
for all to authenticated
using (exists (
  select 1 from public.school_curriculum_versions scv
  where scv.id = school_curriculum_version_id and public.is_school_admin(scv.school_id)
))
with check (exists (
  select 1 from public.school_curriculum_versions scv
  where scv.id = school_curriculum_version_id and public.is_school_admin(scv.school_id)
...
create policy class_curriculum_select_accessible on public.class_curriculum_selections
...
with check (public.is_school_admin(school_id) and public.can_access_class(class_id));
create policy class_curriculum_delete_admin on public.class_curriculum_selections
for delete to authenticated using (public.is_school_admin(school_id));

grant select, insert, update, delete on public.school_curriculum_versions,
  public.school_curriculum_mappings, public.class_curriculum_selections to authenticated;
...
('NPI / MŠMT', 'Elektronický RVP ZV', 'https://prohlednout.rvp.cz/', 'official', 'revised-2025', 'Strukturovaný zdroj očekávaných výsledků a kódů.'),
('NPI ČR', 'Modelové ŠVP pro ZŠ', 'https://revize.rvp.cz/zv/jak-na-svp/modelove-svp-pro-zs', 'official', '2026', 'Modelové ŠVP a podklady pro rozřazení do ročníků.');

insert into public.curriculum_versions (code, name, origin, status, source_id, description)
select 'rvp_zv_2004_current', 'RVP ZV — dosavadní režim', 'official', 'published', id,
  'Dosavadní RVP ZV podporovaný během přechodového období.'
from public.curriculum_sources where source_url like 'https://msmt.gov.cz/%'
union all
select 'rvp_zv_revised_2025', 'RVP ZV — revidovaný', 'official', 'published', id,
  'Revidovaný RVP ZV s elektronickými očekávanými výsledky učení.'
