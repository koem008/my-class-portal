...
-- -----------------------------------------------------------------------------
-- 0. Remove legacy prototype objects.
-- Prototype/demo rows are intentionally not migrated into the privacy-first model.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 5. Teacher profile bootstrap only.
-- No role or school membership is inferred from client-editable profile data.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 7. Safe tenant bootstrap RPC
-- Creates the first school membership without exposing broad INSERT policies.
-- -----------------------------------------------------------------------------
...
begin
  ...
  if char_length(trim(_school_name)) < 1 or char_length(trim(_school_name)) > 160 then
    raise exception 'Invalid school name';
  end if;

  if _academic_year_label !~ '^[0-9]{4}/[0-9]{4}$' then
    raise exception 'Invalid academic year label';
  end if;

  if _ends_on <= _starts_on then
    raise exception 'Invalid academic year dates';
  end if;

  insert into public.schools(name)
  values (trim(_school_name))
...
-- Structural class ownership is admin-only to prevent tenant relocation by a teacher.
