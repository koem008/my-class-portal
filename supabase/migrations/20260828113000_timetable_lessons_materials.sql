...
create table public.timetable_slots (
  ...
  class_id uuid not null references public.classes(id) on delete cascade, academic_year_id uuid not null references public.academic_years(id) on delete cascade,
  weekday smallint not null check (weekday between 1 and 5), slot_order smallint not null check (slot_order between 1 and 12),
  starts_at time not null, ends_at time not null, subject_name text not null, curriculum_subject_id uuid references public.curriculum_subjects(id) on delete set null,
  valid_from date, valid_to date, is_active boolean not null default true, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
  unique (class_id, academic_year_id, weekday, slot_order), check (ends_at > starts_at)
);

create table public.lesson_instances (
  id uuid primary key default gen_random_uuid(), school_id uuid not null references public.schools(id) on delete cascade,
  class_id uuid not null references public.classes(id) on delete cascade, academic_year_id uuid not null references public.academic_years(id) on delete cascade,
  timetable_slot_id uuid references public.timetable_slots(id) on delete set null, lesson_date date not null, slot_order smallint not null check (slot_order between 1 and 12),
  starts_at time, ends_at time, subject_name text not null, curriculum_subject_id uuid references public.curriculum_subjects(id) on delete set null,
  curriculum_topic_id uuid references public.curriculum_topics(id) on delete set null, title text, topic text, status public.lesson_status not null default 'planned',
  source_calendar_event_id uuid references public.calendar_events(id) on delete set null, moved_from_lesson_id uuid references public.lesson_instances(id) on delete set null,
  teacher_note text, created_by uuid not null default auth.uid() references auth.users(id) on delete restrict, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
  unique (class_id, lesson_date, slot_order), check (ends_at is null or starts_at is null or ends_at > starts_at)
);

create table public.lesson_preparations (
  id uuid primary key default gen_random_uuid(), school_id uuid not null references public.schools(id) on delete cascade, class_id uuid not null references public.classes(id) on delete cascade,
  lesson_id uuid not null references public.lesson_instances(id) on delete cascade, objective text, learning_goals jsonb not null default '[]'::jsonb,
  timeline jsonb not null default '[]'::jsonb, teacher_notes text, board_notes text, homework text, reflection text,
  version integer not null default 1 check (version > 0), created_by uuid not null default auth.uid() references auth.users(id) on delete restrict,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (lesson_id, version)
);

create table public.lesson_materials (
  id uuid primary key default gen_random_uuid(), school_id uuid not null references public.schools(id) on delete cascade, class_id uuid not null references public.classes(id) on delete cascade,
  lesson_id uuid not null references public.lesson_instances(id) on delete cascade, kind public.material_kind not null, title text not null, content jsonb not null default '{}'::jsonb,
  ...
  created_by uuid not null default auth.uid() references auth.users(id) on delete restrict, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
alter table public.timetable_slots add constraint timetable_class_school_fk foreign key (school_id,class_id) references public.classes(school_id,id) on delete cascade,
...
alter table public.lesson_instances add constraint lesson_class_school_fk foreign key (school_id,class_id) references public.classes(school_id,id) on delete cascade,
...
alter table public.lesson_preparations add constraint preparation_class_school_fk foreign key (school_id,class_id) references public.classes(school_id,id) on delete cascade,
...
alter table public.lesson_materials add constraint material_class_school_fk foreign key (school_id,class_id) references public.classes(school_id,id) on delete cascade,
...
language sql stable security invoker set search_path = public as $$
  ...
  where ce.affects_schedule = true
    and ce.starts_at < ((_day + 1)::timestamp at time zone 'Europe/Prague')
    and ce.ends_at > (_day::timestamp at time zone 'Europe/Prague')
    and (
      ce.class_id = _class_id
      or (ce.scope = 'school' and exists (select 1 from public.classes c where c.id = _class_id and c.school_id = ce.school_id))
      or (ce.scope = 'private' and ce.created_by = auth.uid())
    )
  union all
  select 'system_day'::text, scd.title, scd.blocks_lessons, null::uuid
  from public.system_calendar_days scd
  where scd.starts_on <= _day and scd.ends_on >= _day;
$$;
...
alter table public.timetable_slots enable row level security; alter table public.lesson_instances enable row level security;
alter table public.lesson_preparations enable row level security; alter table public.lesson_materials enable row level security;
