...
-- This migration intentionally replaces the initial school-portal schema.
-- It does NOT add curriculum, AI, voice, lesson generation or production seed data.

begin;

-- -----------------------------------------------------------------------------
-- 0. Remove legacy policies/triggers/tables from the initial prototype schema.
-- The existing repository contains only prototype/demo data. We do not migrate
-- identity-bearing student rows into the new model.
-- -----------------------------------------------------------------------------
...
create table public.student_aliases (
  ...
  constraint student_aliases_class_same_school
...
-- SECURITY DEFINER functions are deliberately small and locked to search_path.
-- -----------------------------------------------------------------------------
...
as $$
  select exists (
    ...
    where cm.class_id = _class_id
      and cm.user_id = auth.uid()
      and cm.role in ('teacher', 'assistant_teacher')
  );
$$;
...
-- -----------------------------------------------------------------------------
-- 5. RLS
-- No sensitive tenant table uses USING (true).
-- -----------------------------------------------------------------------------
...
-- school_memberships
create policy school_memberships_select_self_or_admin
...
-- classes
create policy classes_select_assigned_or_admin
on public.classes for select to authenticated
using (public.is_school_admin(school_id) or public.can_access_class(id));

create policy classes_insert_admin
on public.classes for insert to authenticated
with check (public.is_school_admin(school_id));

create policy classes_update_assigned_or_admin
on public.classes for update to authenticated
using (public.is_school_admin(school_id) or public.is_class_teacher(id))
with check (public.is_school_admin(school_id) or public.is_class_teacher(id));
...
-- class_memberships
create policy class_memberships_select_assigned_or_admin
...
create policy class_memberships_insert_admin
...
create policy class_memberships_update_admin
...
create policy class_memberships_delete_admin
...
-- student_aliases
create policy student_aliases_select_class
on public.student_aliases for select to authenticated
...
create policy student_aliases_delete_class
on public.student_aliases for delete to authenticated
using (public.can_access_class(class_id));
...
create trigger student_aliases_set_updated_at
before update on public.student_aliases
for each row execute function public.set_updated_at();

-- -----------------------------------------------------------------------------
-- 8. No production seed data.
-- Deliberately no real-looking child names, birth dates, grades, attendance or
-- notes. Test fixtures belong in isolated test tooling, not production schema.
-- -----------------------------------------------------------------------------
