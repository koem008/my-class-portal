...
export async function loadDailyBriefing(classInfo: AccessibleClass, date: string): Promise<DailyBriefing> {
  ...
  const lessonsResult = await db.from("lesson_instances")
    ...
    .eq("class_id", classInfo.id).eq("lesson_date", date).order("slot_order");
  if (lessonsResult.error) throw lessonsResult.error;
  const lessons = (lessonsResult.data ?? []) as LessonInstance[];
  const lessonIds = lessons.map((l) => l.id);

  const [prepResult, materialsResult, eventsResult, systemResult, recentLessonsResult, profileResult] = await Promise.all([
    lessonIds.length ? db.from("lesson_preparations").select("lesson_id").in("lesson_id", lessonIds) : Promise.resolve({ data: [], error: null }),
    lessonIds.length ? db.from("lesson_materials").select("lesson_id").in("lesson_id", lessonIds) : Promise.resolve({ data: [], error: null }),
    db.from("calendar_events").select("id,title,kind,starts_at,ends_at,all_day,blocks_lessons").eq("school_id", classInfo.school_id).lt("starts_at", nextStart).gt("ends_at", dayStart).or(`class_id.eq.${classInfo.id},scope.eq.school,scope.eq.private`).order("starts_at"),
    db.from("system_calendar_days").select("title,kind,starts_on,ends_on,blocks_lessons").lte("starts_on", date).gte("ends_on", date),
    db.from("lesson_instances").select("id,lesson_date,subject_name").eq("class_id", classInfo.id).lt("lesson_date", date).neq("status", "cancelled").order("lesson_date", { ascending: false }).order("slot_order", { ascending: false }).limit(20),
    db.from("teacher_profiles").select("display_name").limit(1).maybeSingle(),
  ]);
  for (const result of [prepResult, materialsResult, eventsResult, systemResult, recentLessonsResult, profileResult]) if (result.error) throw result.error;

  const preparedIds = new Set((prepResult.data ?? []).map((x: any) => x.lesson_id as string));
  const materialCounts = new Map<string, number>();
  for (const row of materialsResult.data ?? []) materialCounts.set((row as any).lesson_id, (materialCounts.get((row as any).lesson_id) ?? 0) + 1);
  const dailyLessons: DailyLesson[] = lessons.map((lesson) => ({ ...lesson, prepared: preparedIds.has(lesson.id), materialCount: materialCounts.get(lesson.id) ?? 0 }));

  const recentIds = (recentLessonsResult.data ?? []).map((x: any) => x.id as string);
  const progressResult = recentIds.length
    ? await db.from("lesson_progress").select("lesson_id,state,unfinished_summary,next_lesson_note").in("lesson_id", recentIds).or("state.eq.partial,unfinished_summary.not.is.null")
    : { data: [], error: null };
  if (progressResult.error) throw progressResult.error;
  const recentById = new Map((recentLessonsResult.data ?? []).map((x: any) => [x.id, x]));
  const carryOvers: DailyCarryOver[] = (progressResult.data ?? []).filter((p: any) => p.unfinished_summary?.trim() || p.state === "partial").map((p: any) => {
    const lesson = recentById.get(p.lesson_id) as any;
    return { lessonId: p.lesson_id, subject: lesson?.subject_name ?? "Výuka", lessonDate: lesson?.lesson_date ?? "", unfinished: p.unfinished_summary?.trim() || "Předchozí hodina zůstala částečně nedokončená.", nextNote: p.next_lesson_note ?? null };
  }).slice(0, 5);

  const events: DailyEvent[] = [
    ...(eventsResult.data ?? []).map((e: any) => ({ id: e.id, title: e.title, kind: String(e.kind), starts_at: e.starts_at, ends_at: e.ends_at, all_day: e.all_day, blocks_lessons: Boolean(e.blocks_lessons), source: "calendar" as const })),
...
export function buildMorningMessage(briefing: DailyBriefing): string {
  const greeting = briefing.teacherDisplayName ? `Dobré ráno, ${briefing.teacherDisplayName}.` : "Dobré ráno.";
  ...
  const lessonText = briefing.lessons.length === 0 ? "Dnes zatím nemáš v rozvrhu žádnou hodinu." : `Dnes tě čeká ${briefing.lessons.length} ${lessonWord(briefing.lessons.length)}.`;
  const prepText = briefing.missingPreparationCount === 0 && briefing.lessons.length > 0 ? " Všechny dnešní hodiny mají uloženou přípravu." : briefing.missingPreparationCount > 0 ? ` ${briefing.missingPreparationCount} ${prepWord(briefing.missingPreparationCount)} ještě chybí připravit.` : "";
  const carryText = briefing.carryOvers.length ? ` Z předchozí výuky zůstává ${briefing.carryOvers.length} ${carryWord(briefing.carryOvers.length)} k navázání.` : "";
  return `${greeting} ${lessonText}${prepText}${carryText}`;
}

function lessonWord(count: number) { return count === 1 ? "hodina" : count >= 2 && count <= 4 ? "hodiny" : "hodin"; }
function prepWord(count: number) { return count === 1 ? "hodinu" : count >= 2 && count <= 4 ? "hodiny" : "hodin"; }
function carryWord(count: number) { return count === 1 ? "věc" : count >= 2 && count <= 4 ? "věci" : "věcí"; }
