...
export type LearningSignalKind =
  "needs_practice" | "improving" | "mastered" | "advanced" | "follow_up";
...
export type ContinuitySuggestion = {
  ...
  detail: string;
  alias?: string;
  source: "current_reflection" | "previous_lesson" | "learning_signal";
};
...
async function loadCurriculumContext(lesson: LessonInstance): Promise<CurriculumContext> {
  ...
  if (!lesson.curriculum_subject_id)
    return {
      linked: false,
      classGrade,
      subject: null,
  ...
  const subjectResult = await db
    ...
    .select("id,code,name,curriculum_version_id,source_id")
    .eq("id", lesson.curriculum_subject_id)
    .maybeSingle();
  ...
  const subject = {
    id: subjectResult.data.id as string,
    code: subjectResult.data.code as string,
    name: subjectResult.data.name as string,
  };
  const topicPromise = lesson.curriculum_topic_id
    ? db
        .from("curriculum_topics")
        .select("id,code,name,description")
        .eq("id", lesson.curriculum_topic_id)
        .maybeSingle()
    : Promise.resolve({ data: null, error: null });
  let outcomesQuery = db
    .from("curriculum_outcomes")
    .select("id,official_code,title,description,target_grade,source_locator")
    .eq("subject_id", subject.id);
  if (lesson.curriculum_topic_id)
    outcomesQuery = outcomesQuery.eq("topic_id", lesson.curriculum_topic_id);
  ...
  const [topicResult, outcomesResult, versionResult] = await Promise.all([
    ...
    db
      ...
      .select("id,code,name,status,source_id")
      .eq("id", subjectResult.data.curriculum_version_id)
      .maybeSingle(),
  ]);
  for (const result of [topicResult, outcomesResult, versionResult])
    if (result.error) throw result.error;
  const sourceId = versionResult.data?.source_id ?? subjectResult.data.source_id;
  const sourceResult = sourceId
    ? await db
        .from("curriculum_sources")
        .select("authority,title,source_url,source_version")
        .eq("id", sourceId)
        .maybeSingle()
  ...
  return {
    ...
    version: versionResult.data
      ? {
          ...
          code: versionResult.data.code,
          name: versionResult.data.name,
          status: String(versionResult.data.status),
        }
      : null,
    source: sourceResult.data
      ? {
          authority: sourceResult.data.authority,
          title: sourceResult.data.title,
          source_url: sourceResult.data.source_url,
          source_version: sourceResult.data.source_version,
        }
      : null,
  };
}
...
export async function loadLessonWorkspace(lessonId: string) {
  ...
  const previousLessonResult = await db
    .from("lesson_instances")
    .select("id,lesson_date,subject_name,topic,status")
    .eq("class_id", lesson.class_id)
    .eq("subject_name", lesson.subject_name)
  ...
  const [
    ...
    db
      ...
      .select(
        "id,lesson_id,objective,learning_goals,timeline,teacher_notes,board_notes,homework,reflection,version",
      )
    ...
    db
      ...
      .select("id,lesson_id,kind,title,content,difficulty,export_status")
      .eq("lesson_id", lessonId)
      .order("created_at", { ascending: true }),
    db
      .from("lesson_progress")
      .select(
        "id,lesson_id,state,completed_summary,unfinished_summary,next_lesson_note,teacher_reflection",
      )
      .eq("lesson_id", lessonId)
      .maybeSingle(),
    previousLessonId
    ...
    db
      ...
      .select(
        "id,lesson_id,student_alias_id,kind,curriculum_outcome_id,topic,note,active,created_at",
      )
      .eq("class_id", lesson.class_id)
      .eq("active", true)
  ...
  const continuitySourceType: "current_reflection" | "previous_lesson" = currentProgress
    ? "current_reflection"
    : "previous_lesson";
  return {
    lesson,
    ...
    signals,
    curriculum,
    continuity: buildContinuitySuggestions(
      continuitySource,
      signals,
      lesson.topic,
...
export async function savePreparation(
  ...
  values: Pick<LessonPreparation, "objective" | "teacher_notes" | "board_notes" | "homework">,
  existingId?: string,
) {
  if (existingId) {
  ...
  const { error } = await db.from("lesson_preparations").insert({
...
export async function createMaterial(
  ...
  input: {
    ...
    title: string;
    text: string;
    difficulty?: LessonMaterial["difficulty"];
  },
) {
...
export async function applyArtThemeToLesson(
  lesson: LessonInstance,
  theme: ArtThemeInput,
  existingPreparationId?: string,
) {
  const objective = theme.objectives.join("\n");
  const teacherNotes = [
    theme.summary,
    "",
    `Pomůcky: ${theme.materials.join(", ")}`,
    "",
    ...theme.steps.map((step, i) => `${i + 1}. ${step}`),
    theme.easyVariant ? `\nLehčí varianta: ${theme.easyVariant}` : "",
    theme.advancedVariant ? `\nRozšířená varianta: ${theme.advancedVariant}` : "",
    theme.reflectionPrompt ? `\nReflexe: ${theme.reflectionPrompt}` : "",
    theme.rvpCodes.length ? `\nVazba na RVP: ${theme.rvpCodes.join(", ")}` : "",
  ]
    .filter(Boolean)
    .join("\n");
  await savePreparation(
    lesson,
    { objective, teacher_notes: teacherNotes, board_notes: theme.title, homework: null },
  ...
  const existingMaterial = await db
    ...
    .maybeSingle();
  if (existingMaterial.error) throw existingMaterial.error;
...
export async function saveProgress(
  lesson: LessonInstance,
  values: Omit<LessonProgress, "id" | "lesson_id">,
  existingId?: string,
) {
  if (existingId) {
    const { error } = await db
...
export async function createLearningSignal(
  ...
  input: {
    ...
    topic?: string;
    note?: string;
    curriculumOutcomeId?: string | null;
  },
) {
...
export function buildContinuitySuggestions(
  ...
  progressSource: "current_reflection" | "previous_lesson" = "current_reflection",
  ...
  if (progress?.state === "partial" || progress?.unfinished_summary?.trim())
    suggestions.push({
      ...
      detail:
        progress.unfinished_summary?.trim() || "Předchozí hodina byla dokončena jen částečně.",
      source: progressSource,
    });
  if (progress?.next_lesson_note?.trim())
    suggestions.push({
      priority: "high",
      title: "Poznámka pro příští hodinu",
      detail: progress.next_lesson_note.trim(),
      source: progressSource,
    });
  const relevantSignals = signals.filter(
    (signal) =>
      !topic ||
      !signal.topic ||
      signal.topic.toLocaleLowerCase("cs-CZ") === topic.toLocaleLowerCase("cs-CZ"),
  );
  ...
  for (const signal of latestByAlias.values()) {
    ...
    if (signal.kind === "needs_practice")
      suggestions.push({
        priority: "high",
        title: `${alias}: potřebuje procvičit`,
        detail: signal.note || signal.topic || "Připravit kratší a jednodušší procvičení.",
        alias,
        source: "learning_signal",
      });
    if (signal.kind === "follow_up")
      suggestions.push({
        priority: "medium",
        title: `${alias}: vrátit se k tématu`,
        detail: signal.note || signal.topic || "Ověřit porozumění v další hodině.",
        alias,
        source: "learning_signal",
      });
    if (signal.kind === "advanced")
      suggestions.push({
        priority: "low",
        title: `${alias}: rozšiřující úloha`,
        detail: signal.note || signal.topic || "Nabídnout náročnější variantu bez zdržení třídy.",
        alias,
        source: "learning_signal",
      });
    if (signal.kind === "mastered")
      suggestions.push({
        priority: "low",
        title: `${alias}: zvládnuto`,
        detail: signal.note || "Není potřeba základní procvičování; lze nabídnout rozšíření.",
        alias,
        source: "learning_signal",
      });
  }
