export type AssistantMode =
  "morning" | "lesson" | "week" | "materials" | "reflection" | "companion";
...
const forbiddenKeys =
  /(^|_)(name|first_name|last_name|email|phone|address|birth_date|rodne_cislo|personal_id)($|_)/i;
export function assertPrivacySafe(value: unknown, path = "context"): void {
  if (Array.isArray(value)) {
    value.forEach((v, i) => assertPrivacySafe(v, `${path}[${i}]`));
    return;
  }
  if (!value || typeof value !== "object") return;
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    if (forbiddenKeys.test(key))
      throw new Error(`Privacy contract rejected forbidden field at ${path}.${key}`);
