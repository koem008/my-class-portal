    ...
    <main className="relative min-h-screen overflow-hidden bg-[#f8f3ea] px-4 py-7 text-[#26363b] sm:px-6 md:py-10">
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="mb-6 flex items-center justify-between gap-4">
          <Link
            to="/"
            className="text-xs font-black text-[#7c8983] transition hover:text-[#2c756f]"
          >
            ← zpátky
          </Link>
        ...
        <div className="grid gap-6 lg:grid-cols-[.78fr_1.22fr] lg:gap-8">
          <aside className="relative overflow-hidden rounded-[38px] bg-[#2c756f] p-6 text-white shadow-[0_28px_70px_rgba(44,117,111,.22)] sm:p-8 lg:sticky lg:top-8 lg:self-start">
            ...
            <div className="relative">
              ...
              <p className="mt-4 text-sm leading-7 text-white/72">
                Vyplň jen to, co dává smysl pro tebe. Zbytek se bude skládat postupně podle rozvrhu,
                hodin a toho, co si sem začneš ukládat.
              </p>

              <div className="mt-7 space-y-3">
                <LittleNote
                  tone="bg-[#f7dcbf] text-[#795338]"
                  text="Rozvrh nebude jen tabulka. Každá hodina může mít vlastní přípravu."
                />
                <LittleNote
                  tone="bg-[#dcefe7] text-[#2f685f]"
                  text="Asistentce můžeš psát i mluvit, když nechceš všechno vypisovat."
                />
                <LittleNote
                  tone="bg-[#e8e0f5] text-[#67598b]"
                  text="Citlivé věci zůstanou oddělené. Ty určuješ, co sem patří."
                />
              </div>
          ...
          <section className="relative rounded-[38px] border border-white/80 bg-[#fffdf8]/92 p-5 shadow-[0_28px_80px_rgba(78,69,56,.12)] backdrop-blur-xl sm:p-8">
            ...
            <div className="text-[11px] font-black uppercase tracking-[.18em] text-[#759088]">
              Začneme jednoduše
            </div>
            <h2 className="mt-2 text-3xl font-black tracking-[-.045em] text-[#233238]">
              Co má být tvoje?
            </h2>
            ...
            <div className="mt-7 space-y-5">
              ...
              <div className="pt-1">
                <div className="mb-3 text-xs font-black text-[#667874]">
                  A co z toho je opravdu tvoje práce?
                </div>
                <div className="grid gap-3">
                  <Choice
                    ...
                    text="Přidá vlastní kreativní studio, inspirace a materiály k výuce."
                    tone="peach"
                  />
                  <Choice
                    ...
                    text="Přidá oddělený citlivý pracovní prostor s vlastními bezpečnostními pravidly."
                    tone="lavender"
                  />
                </div>
              </div>
            ...
            <div className="mt-8 rounded-[28px] bg-gradient-to-r from-[#eef7f2] via-[#fff6e9] to-[#f1ecfa] p-4 sm:flex sm:items-center sm:justify-between sm:gap-4">
              <div>
                ...
                <div className="mt-1 text-xs leading-5 text-[#82908c]">
                  Rozvrh, třída a zbytek prostoru se otevřou podle toho, co sis zvolila.
                </div>
              </div>
              <button
...
function Field({
  ...
  const ring =
    accent === "mint"
      ? "focus:border-[#8db9ae] focus:ring-[#e7f3ee]"
      : accent === "peach"
        ? "focus:border-[#d8a47e] focus:ring-[#fff0e5]"
        : "focus:border-[#aaa0cf] focus:ring-[#f0ecfa]";
  return (
    <label className="block">
      <div className="flex items-center justify-between">
        <span className="text-xs font-black text-[#5f7370]">{label}</span>
        {optional && <span className="text-[11px] text-[#9aa3a1]">klidně až později</span>}
      </div>
      <input
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className={`mt-2 w-full rounded-[20px] border border-[#e5ded2] bg-white px-4 py-3.5 text-sm shadow-[0_5px_16px_rgba(75,68,57,.04)] outline-none transition focus:ring-4 ${ring}`}
      />
    </label>
  );
}
...
function Locked({ label, value, tone }: { label: string; value: string; tone: string }) {
  return (
    <div className={`rounded-[20px] border border-white px-4 py-3.5 ${tone}`}>
      <div className="text-[11px] font-black text-[#87928e]">{label}</div>
...
function Choice({
  ...
  const active =
    tone === "peach" ? "border-[#ecc5a7] bg-[#fff2e6]" : "border-[#cfc4e8] bg-[#f3effb]";
  return (
    <label
    ...
    >
      ...
      />
      <div
        className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${tone === "peach" ? "bg-[#f7d6bd] text-[#915c3e]" : "bg-[#e0d8f2] text-[#675a8d]"}`}
      >
