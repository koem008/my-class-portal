...
import { AlertCircle, CalendarDays, CheckCircle2, ChevronRight, Clock3, Heart, Loader2, Mic, MoonStar, Settings2, Sparkles, SunMedium } from "lucide-react";
...
function AssistantPage() {
  ...
  return <main className="min-h-screen bg-[#fbfaf7] px-4 py-5 text-[#24343f] md:px-8 md:py-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 rounded-[34px] border border-[#e8e3d9] bg-gradient-to-br from-white via-[#fffaf2] to-[#eaf6f0] p-6 shadow-[0_22px_70px_rgba(66,82,73,.1)] md:p-9">
      {loadState==="loading" ? <div className="flex items-center gap-3 text-[#607572]"><Loader2 className="h-5 w-5 animate-spin"/>Skládám dnešní přehled ze skutečných dat…</div> : loadState==="error" ? <div className="flex items-start gap-3"><AlertCircle className="mt-0.5 h-5 w-5 text-[#b56562]"/><div><div className="font-bold">Dnešní přehled se nepodařilo načíst.</div><p className="mt-1 text-sm text-[#778685]">{error}</p><button onClick={()=>void reload()} className="mt-3 rounded-xl bg-white px-3 py-2 text-xs font-bold text-[#276765]">Zkusit znovu</button></div></div> : loadState==="empty" ? <div><div className="text-xs font-bold uppercase tracking-[.16em] text-[#5b817c]">Ranní briefing</div><h2 className="mt-2 text-2xl font-bold">Dobré ráno.</h2><p className="mt-3 text-sm leading-6 text-[#627477]">Zatím nemáš přiřazenou žádnou třídu. Jakmile ji nastavíš, tady se automaticky objeví skutečný rozvrh, události a návaznosti.</p></div> : briefing && <>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[1.35fr_.65fr]"><section className="rounded-[30px] border border-[#e9e5dd] bg-white p-5 md:p-7"><h2 className="font-bold">Řekni mi, co potřebuješ</h2><p className="mt-1 text-xs text-[#83908f]">Hlas se spouští pouze po stisknutí tlačítka.</p><button onClick={()=>setListening(!listening)} className={`mx-auto mt-8 grid h-28 w-28 place-items-center rounded-full text-white shadow-[0_18px_45px_rgba(39,103,101,.25)] transition ${listening?"scale-105 bg-[#b85f61]":"bg-[#276765] hover:scale-105"}`}><Mic className="h-9 w-9"/></button><p className="mt-4 text-center text-sm font-semibold text-[#53696a]">{listening?"Mikrofon zatím není připojený — klepnutím zavři":"Klepni pro hlasové zadání"}</p><div className="mt-7 grid gap-2 sm:grid-cols-2">{["Připrav mi zítřejší matematiku","Co ještě dnes musím dodělat?","Navážeme na poslední nedodělek","Vytvoř pracovní list k další hodině"].map(x=><button key={x} disabled className="flex items-center justify-between rounded-2xl bg-[#f8f7f3] px-4 py-3 text-left text-sm text-[#8c9695]">{x}<ChevronRight className="h-4 w-4"/></button>)}</div><div className="mt-6 rounded-2xl border border-dashed border-[#ddd8ce] bg-[#fcfbf8] p-4 text-xs leading-5 text-[#7b8989]"><strong>AI a hlas zatím nejsou připojené.</strong> Reálný pracovní kontext už aplikace sestavuje sama; až doplníme serverový API klíč, pošle se providerovi jen minimum nutných dat.</div></section><aside className="space-y-4"><section className="rounded-[28px] border border-[#e9e5dd] bg-white p-5"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Settings2 className="h-5 w-5"/><h3 className="font-bold">Osobní paměť</h3></div><button onClick={()=>setMemory(!memory)} className={`h-7 w-12 rounded-full p-1 ${memory?"bg-[#276765]":"bg-[#d8dcda]"}`}><span className={`block h-5 w-5 rounded-full bg-white transition ${memory?"translate-x-5":""}`}/></button></div><p className="mt-2 text-xs leading-5 text-[#7b8989]">{memory?"Zapnutá pouze v tomto rozhraní; trvalé ukládání osobních preferencí ještě není aktivní.":"Vypnutá. Asistentka používá pouze pracovní kontext."}</p></section><Mini icon={Heart} title="Soukromí" text="Žáci jsou pro AI pouze pseudonymy. Skutečné identity se do kontextu neposílají."/></aside></div>
  </div></main>;
}
